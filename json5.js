...
module.exports = {
    translate: function(load) {
        var json = JSON5.parse(load.source);
        if (this.builder && this.transpiler && !Array.isArray(json)) {
            ...
            validIdentifiers.forEach(function (exportName) {
                output.push('exp' + 'ort var ' + exportName + ' = ' + JSON5.stringify(json[exportName]) + ';\n');
            });
            ...
            namedExports.forEach(function (exportName) {
                if (validIdentifiers.indexOf(exportName) !== -1) {
                    ...
                    output.push(JSON5.stringify(exportName) + ': ' + JSON5.stringify(json[exportName]) + ',\n');
                }
            });
        ...
        if (this.builder) {
            ...
            return 'module.exports = ' + JSON5.stringify(json);
        }
    },
    instantiate: function(load) {
        if (!this.builder) {
            return JSON5.parse(load.source);
        }
    }
}
